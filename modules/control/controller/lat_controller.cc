...
Status LatController::Init(std::shared_ptr<DependencyInjector> injector,
  ...
  */
  ...
  matrix_a_coeff_(1, 1) = -(2*cf_+2*cr_)/(mass_ * init_vehicle_x_);
  matrix_a_coeff_(1, 3) = (-2*cf_*lf_+2*cr_*lr_)/(mass_ * init_vehicle_x_);
  matrix_a_coeff_(3, 1) = (-2*cf_*lf_+2*cr_*lr_)/(iz_ * init_vehicle_x_);
  matrix_a_coeff_(3, 3) = -(2*cf_*lf_+2*cr_*lr_*lr_)/(iz_ * init_vehicle_x_);
